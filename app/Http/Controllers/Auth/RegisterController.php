...
use App\Http\Controllers\Auth\Throwable;
use App\Models\User;
...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function showFormKomunitas()
    {
        // $data['komunitas'] = Komunitas::whereHas('user', function ($query) {
        //     $query->where('active', 1);
        // })->get();
        // $data['komunitas'] = Komunitas::all();
        return view('auth.register-komunitas');
    }

    public function registerKomunitas(Request $request)
    {
        try {
            ...
            if (!$validator->fails()) {
                ...
                // Lakukan proses pendaftaran Komunitas di sini
                // Anda dapat menggunakan $data untuk mengakses data yang diinputkan
                ...
                $user = new User([
                    'uuid' => generateUuid(), // Pastikan Anda telah mendefinisikan generateUuid() sesuai dengan kebutuhan Anda.
                    'email' => $data['email'],
                    'nama' => $data['nama'],
                    'password' => $data['password'],
                    'role' => 'komunitas', // Sesuaikan dengan role yang sesuai
                ]);

                // $user = $user->save();

                $user = $komunitas->user()->save($user);
                ...
                $key = generateUuid(); // Tambahkan ini untuk menghasilkan kunci cookie

                // Tambahkan cookie dengan $key dan email
                Cookie::queue($key, $data['email'], 10);
                DB::commit();
            ...
            return redirect()->back()->with('message', 'Permintaan anda tidak dapat diproses, silahkan ulangi kembali.');
        }
    }

    public function showFormKampus()
    {
        return view('auth.register-kampus');
    }
    ...
    public function registerMahasiswa(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'kampus' => 'required|exists:sekolahs,npsn',
            'email' => 'required|email|unique:users,email',
            'password' => 'required|confirmed'
        ]);
    }
